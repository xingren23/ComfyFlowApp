...
class ComfyClient:
    ...
    def get_node_class(self):
        ...
        resp = requests.get(object_info_url, timeout=self.timeout)
    ...
    def upload_image(self, imagefile, subfolder, type, overwrite):
        ...
        resp = requests.post(f"http://{self.server_addr}/upload/image", data=data, files=imagefile, timeout=self.timeout)
        if resp.status_code != 200:
    ...
    def get_history(self, prompt_id):
        ...
        resp = requests.get(f"http://{self.server_addr}/history/{prompt_id}", timeout=self.timeout)
        if resp.status_code != 200:
